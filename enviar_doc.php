...
if (!$host || !$login || $senha === '') {
    echo json_encode(['ok' => false, 'msg' => 'Conta de envio incompleta (servidor, login e senha).']);
    exit;
}
// Aceita múltiplos destinatários separados por vírgula ou ponto-e-vírgula
$lista_para = preg_split('/[;,]+/', $para);
$lista_para = array_values(array_filter(array_map('trim', $lista_para), function ($e) {
    return $e !== '';
}));
if (count($lista_para) === 0) {
    echo json_encode(['ok' => false, 'msg' => 'Informe ao menos um e-mail de destino.']);
    exit;
}
foreach ($lista_para as $e) {
    if (!preg_match('/.+@.+\..+/', $e)) {
        echo json_encode(['ok' => false, 'msg' => 'E-mail de destino inválido: ' . $e]);
        exit;
    }
}
...
if (!$fp) {
    echo json_encode(['ok' => false, 'msg' => 'Não conectou em ' . $host . ':' . $porta . ' — ' . $errstr . ' (' . $errno . ')', 'log' => $log]);
    exit;
}
...
if (substr($resp, 0, 3) !== '220') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'Servidor não respondeu 220 (saudação). Resposta: ' . trim($resp), 'log' => $log]);
    exit;
}
...
if ($seg === 'tls') {
    ...
    if (substr($r, 0, 3) !== '220') {
        fclose($fp);
        echo json_encode(['ok' => false, 'msg' => 'STARTTLS falhou: ' . trim($r) . ' — tente a porta 465 com SSL.', 'log' => $log]);
        exit;
    }
    if (!@stream_socket_enable_crypto($fp, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
        fclose($fp);
        echo json_encode(['ok' => false, 'msg' => 'Não foi possível ativar a criptografia TLS.', 'log' => $log]);
        exit;
    }
    _cmd($fp, 'EHLO mh3rental.com.br', $log);
}
...
if (substr($r, 0, 3) !== '334') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'Servidor não aceitou AUTH LOGIN: ' . trim($r), 'log' => $log]);
    exit;
}
$r = _cmd($fp, base64_encode($login), $log);
if (substr($r, 0, 3) !== '334') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'Login não aceito nesta etapa: ' . trim($r), 'log' => $log]);
    exit;
}
$r = _cmd($fp, base64_encode($senha), $log, false);
if (substr($r, 0, 3) !== '235') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'Autenticação recusada (login ou senha incorretos): ' . trim($r), 'log' => $log]);
    exit;
}
...
if (substr($r, 0, 3) !== '250') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'Remetente ' . $remetente . ' recusado: ' . trim($r), 'log' => $log]);
    exit;
}
foreach ($lista_para as $dest) {
    ...
    if (substr($r, 0, 3) !== '250' && substr($r, 0, 3) !== '251') {
        fclose($fp);
        echo json_encode(['ok' => false, 'msg' => 'Destino ' . $dest . ' recusado: ' . trim($r), 'log' => $log]);
        exit;
    }
}
$r = _cmd($fp, 'DATA', $log);
if (substr($r, 0, 3) !== '354') {
    fclose($fp);
    echo json_encode(['ok' => false, 'msg' => 'DATA recusado: ' . trim($r), 'log' => $log]);
    exit;
}
...
$to_header = implode(', ', array_map(function ($e) {
    return '<' . $e . '>';
}, $lista_para));
$msg .= "To: " . $to_header . "\r\n";
        ...
        if ($mbox) {
            ...
            if (is_array($lista)) {
                foreach ($lista as $l) {
                    ...
                    if (strpos($low, 'sent') !== false || strpos($low, 'enviad') !== false) {
                        $pasta_sent = $n;
                        break;
                    }
                }
